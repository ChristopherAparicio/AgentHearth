...
Every provider source AgentHearth reads is undocumented: Claude's transcripts,
its account-global usage journal and its Keychain profiles, Codex's rollouts and
rate-limit families. Providers reshape them without notice, and unit tests over
fixtures cannot notice — the fixture keeps matching the old shape forever.

`ProviderFormatContractTests` closes that gap by replaying the real parsers over
whichever of those sources exist on the machine running the suite, asserting only
that data still comes out. A drift then names the source that stopped parsing
instead of surfacing later as a wrong figure in the menu. A source that is absent
skips rather than fails, so CI and a fresh checkout stay green, and the tests read
no secret material: Keychain items are enumerated by attribute only, never by
value.
