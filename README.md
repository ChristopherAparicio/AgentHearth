...
![The AgentHearth dashboard: usage limits per provider in the macOS menu bar](Docs/screenshots/dashboard.png)

The dashboard leads with each provider's usage limits — the reset times
included — and keeps the full session list one click away in that provider's
own tab.

New here? [Docs/OVERVIEW.md](Docs/OVERVIEW.md) explains what the app is for and
what it shows you, in plain language.
...
![Cache Insights: token-weighted cache reuse over 30 days, with the projects reprocessing the most uncached input](Docs/screenshots/cache-insights.png)
...
See [the overview](Docs/OVERVIEW.md) for what the app is for in plain language,
