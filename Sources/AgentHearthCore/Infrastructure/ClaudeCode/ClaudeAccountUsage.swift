...
public enum ClaudeAccountUsageDecoder {
    ...
    /// Per-model weekly allowances arrive in `limits` as `weekly_scoped`
    /// entries. They are frequently the binding constraint, so each becomes its
    /// own window rather than being folded into the account-wide one.
    private static func scopedWeekly(from limits: [RawLimit]) -> [AccountUsage.ScopedWindow] {
        ...
        return limits.compactMap { limit in
            guard limit.kind == "weekly_scoped",
                  ...
                  let label = limit.scope?.model?.displayName?
    ...
    /// One entry of `limits`. Only the fields AgentHearth reads are declared, so
    /// unknown kinds still decode and are simply ignored.
