...
struct ProviderCardView: View {
    ...
    /// The Dashboard tab splits a provider into two card flavors: a usage
    /// summary (header + limit bars, no sessions) and a priority card (pinned
    /// sessions only). Both keep a clickable header that opens the provider's
    /// own tab for the full list.
    var content: ProviderCardContent = .full
    var onSelectProvider: ((AgentProviderID) -> Void)?

    private var displaySessions: [AgentSession] {
        switch content {
        case .full: snapshot.sessions
        case .usageSummary: []
        case .prioritySessions: snapshot.sessions.filter(isPinned)
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if showsUsageWindows, !snapshot.usageWindows.isEmpty {
            ...
            if showsUsageWindows, !snapshot.usageWindows.isEmpty, !displaySessions.isEmpty {
                Divider()
                    .padding(.vertical, 11)
            }
            ...
            if !displaySessions.isEmpty {
                ...
                .padding(.vertical, -8)
            } else if snapshot.usageWindows.isEmpty || !showsUsageWindows {
    ...
    private var header: some View {
        if content != .full, let onSelectProvider {
            Button {
            ...
            .buttonStyle(.plain)
            .help("Open the \(snapshot.id.displayName) tab")
        } else {
    ...
    private var priorityCountLabel: String {
        let count = displaySessions.count
        return count == 1 ? "1 priority" : "\(count) priority"
    }
...
/// Which slice of a provider a card renders — see `ProviderCardView.content`.
enum ProviderCardContent: Equatable {
    case full
    case usageSummary
